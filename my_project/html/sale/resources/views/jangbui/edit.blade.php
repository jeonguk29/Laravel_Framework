...
<table class="table table-bordered table-sm mymargin5">
	   ...
	   <tr>
		...
		<input type="hidden" name="products_id42" value="{{$row->products_id42}}">
                  <select name="sel_products_id42" class="form-select form-select-sm" onchange="select_product();">
							...
							<option value="{{$row -> products_id42}}" selected>선택하세요.</option>
							@foreach ($list as $row1)
							<?
							 $t1="$row1->id^^$row1->price42";
							 $t2="$row1->name42($row1->price42)";
							
							?>
								@if ( $row1->id == $row->products_id42 )
    ...
    <tr>
	   ...
	   <tr>
        ...
        <td width="80%" align="left">
            <div class="fd-inline-flex">
                <input  type="text" name="numi42" size="20" maxlength="20" value="{{$row -> numi42}}"
                         class="form-control form-control-sm" onChange= "cal_prices();">
            </div>
        </td>
    </tr>
