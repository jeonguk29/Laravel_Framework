...
<script>
    function find_text()
    {
        form1.action="{{route('officervacation.index')}}";
        form1.submit();
    }
...
<form name="form1"  method="get" action="" >
    <div class="row">
        ...
        <div class="col-9" align="right">           
               <a href="{{ route('officervacation.create') }}{{$tmp}}" class="btn btn-sm mycolor1">추가</a>
        </div>
    </div>
</form>
	</div>
...
<table class="table  table-bordered  table-sm  table-hover mymargin5">
  ...
  @foreach ($list as $row)       
    ...
    <tr>
		...
		<td align="left">
		<a href="{{route('officervacation.show', $row->id)}}{{$tmp}}">
			{{ $row->uids_id42 }}
			</a>
		</td>
		<td>{{ $row->name42 }}</td>
